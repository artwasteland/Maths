# Cross-check: enum2.py replays udenum shards (CONTRACT 6.2(b), PRUNING.md 3.5)

Written 2026-09-05 by the cross-check builder. Section 1 was written BEFORE any replay was run
(it records what was read, not what was measured); the later sections record the run.

Nothing under `enum/`, `enum2/enum2.py` or `data/` was modified. All new files are under
`enum2/crosscheck/` plus this document.

## 1. What enum2.py is, read before running (interface and semantics)

### 1.1 Interface

`enum2/enum2.py` (Python 3.12, `~/tools/pyenv-maths/bin/python`, pynauty 2.8.8.1 which bundles
nauty 2.8.8; `enum/udenum` links the system `libnauty-2.8.8.so`, so both enumerators use the
same nauty release).

The shard-replay entry point is

    enum2.py enumerate --parents FILE --n N --m M --output OUT [--drop-pattern I]... [--break-orbit] [--verify]

- `--parents FILE`: canonical graph6, one record per line, every record on N-1 vertices (a
  record on any other order raises `ValueError`). There is NO `--start/--end` parent range: the
  program consumes the whole file. To replay a udenum shard (a `[start, end)` range of a frozen
  parent file) the driver has to slice the parent file itself. enum2 does NOT check that the
  parent records are canonical (udenum does: `canonical_parent_and_group` exits 2 on a
  non-canonical record); this cannot change the output on canonical input.
- `--n N --m M`: the target cell. For every parent H the new-vertex degree is `d = M - m(H)`
  computed per record from the record's own edge count (`extend_parent`), exactly as udenum's
  `process_parent` does (`d = cfg->target_m - parent_m`). Both therefore accept a parent file
  that concatenates several parent levels, which is what `_parents/<n>-<m>.g6` is.
- `--output OUT`: `write_graph6` writes `sorted(set(records))`, one canonical graph6 per line;
  the process prints `children=<count>` (the count of DISTINCT records). udenum writes children in
  generation order (parent order, then ascending neighbourhood mask) and never deduplicates.
  The comparison below is therefore on SETS of lines, with an explicit check that the udenum
  shard output contains no duplicate line (it must not, by the canonical-augmentation argument in
  1.3).
- The forbidden list is loaded once at import time from the fixed path `data/forbidden-74.json`
  (module constants `LOAD_FORBIDDEN`, 74 graphs, and `PATTERNS`, 635 rooted patterns); there is
  no command-line option to point at another list. Control (b) below therefore drives enum2 as a
  module and rebinds `PATTERNS` from a copied, shortened list (the CLI's `--drop-pattern` would
  do the same thing at pattern level).
- No manifest, no hashing, no sharding, no multiprocessing: single-threaded, one process per
  file. `KNOWN_UBAR` / `edge_window` are used only by the recursive `table`/`check` drivers, not
  on the `enumerate` path; the edge windows for the replay come entirely from which parent
  records udenum's `_parents` files contain (those files were assembled by `run-dag2.py` from
  `reach.json`'s L4 table: e.g. `_parents/12-20.g6` = `11-17.reach.g6` (d=3) + `11-18.reach.g6`
  (d=2), the aggregate manifest's `parent_sources`).

### 1.2 Canonical form and orbits

- enum2: `pynauty.canon_label(g)` (densenauty, default options, `getcanon`) gives the canonical
  order (`order[i]` = old vertex at canonical position i); enum2 relabels its own bitmask
  adjacency and encodes graph6 with its own encoder (`graph6_encode`). udenum: `densenauty`
  with `DEFAULTOPTIONS_GRAPH`, `getcanon = TRUE`, `defaultptn = TRUE`, then nauty's `ntog6` on
  `canong`. Same library release, same options, so the strings should agree byte for byte;
  REPORT.md already showed enum2's canonical strings equal `nauty-labelg`'s on the 56 known
  graphs. Any disagreement of canonical STRINGS (as opposed to sets of isomorphism classes)
  would show up in the comparison as a difference and would be diagnosed by `labelg`.
- Orbits: enum2 uses `pynauty.autgrp(g)[3]` (orbit representative per vertex); udenum uses the
  `orbits` array returned by the same `densenauty` call. Same semantics.

### 1.3 Acceptance rule (canonical augmentation)

Both accept a child G = H + v iff v lies in the Aut(G)-orbit of the FIRST minimum-degree vertex
of G in nauty's canonical order:

- udenum `child_canonical_accept`: `delta` = min degree of G; `canonical_minimum` = `lab[i]` for
  the smallest i with `degree[lab[i]] == delta`; accept iff `orbits[new_vertex] ==
  orbits[canonical_minimum]`.
- enum2 `canonical_augmentation_ok`: `mindeg` = min degree of G; `first` = first `v` in
  `canon_label` order with `degree(v) == mindeg`; accept iff `orbits[added] == orbits[first]`.

Identical statement. Two consequences used by the comparison: (i) an accepted child has
`deg(v) = delta(G) = d`; (ii) if two accepted children of the SAME parent H are isomorphic,
the isomorphism can be composed with an automorphism of G to map new vertex to new vertex,
and its restriction is an automorphism of H carrying one neighbourhood to the other. So
udenum's Aut(H)-orbit representatives of neighbourhoods (`build_neighbourhood_orbits`,
representative = smallest mask) and enum2's "try every neighbourhood, deduplicate the canonical
strings with a set" produce the same set of children per parent; and accepted children of two
non-isomorphic parents are never isomorphic (else an automorphism of G would carry one deleted
vertex to the other and H1 = H2), so a shard's output has no duplicates and the set comparison
is the right comparison.

### 1.4 Parent guard (AMP p. 5) and neighbourhood generation

- udenum `process_parent`: skip if `d < 0` or `d > n(H)`; skip if `delta(H) <= d-2`; if
  `delta(H) == d-1` every minimum-degree vertex is `required` in the neighbourhood and the parent
  is skipped if there are more than d of them.
- enum2 `extend_parent`: return if `min_degree < degree - 1` (same as `delta <= d-2`); if
  `min_degree == degree - 1`, `mandatory` = all minimum-degree vertices, return if
  `mandatory.bit_count() > degree`; `child_neighborhoods` returns nothing for `degree < 0` or
  `degree > n(H)`. Same rule, evaluated in a different order with the same effect.

### 1.5 F-freeness of children (bad neighbourhoods)

Neither enumerator tests children against F directly in normal mode. Both build, for each
parent H, the minimal family of bad vertex sets T (a new vertex adjacent to all of T creates a
member of F) from the 635 rooted patterns (F - x, N_F(x)), one per (F, x), and reject a
neighbourhood iff it contains a bad set:

- udenum: `build_bad_family` enumerates monomorphic images of every pattern with |S| <= d
  (`find_bad_matches`, image of S = bad set, minimality by `bad_family_contains_subset`); when
  `required` is non-empty it uses `build_restricted_bad_family` instead, which walks candidate
  sets of size 2..d containing `required` and tests `contains_forbidden(H + v(T))` directly.
  For neighbourhoods containing `required` that yields the same accept/reject decision (if
  H + v(N) contains F through T = image of N_F(x), then T ∪ required ⊆ N is a tested set, and
  it or a subset of it is in the family).
- enum2: `bad_neighborhoods` for patterns with |S| <= d: `_pattern_neighborhood_images` assigns
  the S vertices first and then only checks that the rest of the pattern embeds (existential);
  minimal family by subset test; `child_neighborhoods` walks all d-subsets containing
  `mandatory`.

Different algorithms, same defined family, which is exactly what the cross-check is for.
Optimisations that cannot change the result on either side: udenum skips patterns with more
vertices or edges than H; enum2 skips patterns with more vertices than H.

### 1.6 Semantic differences found (findings, not failures)

1. enum2 has no parent-range option and no manifest; the driver slices ranges and hashes.
2. enum2 does not verify parent canonicality; udenum refuses a non-canonical parent (exit 2).
   Consequence for control (a): a parent with one flipped edge is normally NOT canonical, so
   udenum could not even have run on it; the control canonicalises the mutated record first so
   both enumerators would accept it as input.
3. enum2 deduplicates output with a set and sorts it; udenum emits in generation order without
   deduplication. Equivalent under the argument of 1.3, which the replay checks empirically
   (duplicate-line check on every udenum shard, count of raw vs distinct enum2 children).
4. enum2 loads the forbidden list from a fixed path at import time; udenum takes `--forbidden`.
5. Neither difference in 1.4/1.5 changes the defined output; none was found in the acceptance
   rule itself.

## 2. The fraction replayed, and why it is a sample

### 2.1 Measured speed of enum2 (the reason a whole transition was not replayed)

Bench command: `replay.py bench --parent-file <_parents file> --start A --end B --n N --m M`
(one process; measured with four such processes running concurrently under `nice -n 10` on
liam-desktop, 4 cores, load already ~3.7 from other sessions). Seconds per parent that udenum's
guard does not skip:

| region | hosts | d | measured | parents/s |
|---|---|---|---|---|
| `_parents/12-20.g6` [200000,200200) | n=11 | 2 | 200 parents, 80.5 s | 2.5 |
| `_parents/12-22.g6` [40000,40020) | n=11 | 3 | 20 parents, 108.7 s | 0.18 |
| `_parents/12-20.g6` [0,1000) (5% processed) | n=11 | 3 | 1000 parents, 294.8 s | 3.4 |
| `_parents/12-24.g6` [0,150) (14% processed) | n=11 | 4 | 150 parents, 27.9 s | 5.4 |
| `_parents/12-21.g6` [270000,270200) | n=11 | 1 | 200 parents, 0.9 s | 222 |
| `_parents/11-17.g6` [10000,10300) | n=10 | 2 | 300 parents, 56.5 s | 5.3 |
| `_parents/11-20.g6` [0,30) | n=10 | 3 | 30 parents, 85.8 s | 0.35 |
| `_parents/11-17.g6` [0,300) (all skipped by the guard) | n=10 | 3 | 300 parents, 0.3 s | 1086 |

udenum does the same work at 6,000-10,000 parents per second per core (its shard stats:
10,000 dense d=2 parents of 12-20 in 1.6 s). enum2 is 2,000-30,000 times slower, the d=3 path
(`required` non-empty, 474 patterns of |S| <= 3 instead of the 61 of |S| = 2) being the worst.
The two transitions 10 -> 11 and 11 -> 12 hold 815,215 parent records (97,249 + 717,966), of
which udenum processed 636,488 (the rest are skipped by the minimum-degree guard), producing
5,128,203 children. At the measured rates a complete replay of the 11 -> 12 transition alone is
of the order of 100 core-hours; the budget for this item was about 90 minutes of wall time on 4
cores. So the replay is a SAMPLE, chosen as follows.

### 2.2 The sample (seed 20260905, `plan-main.json`)

`replay.py plan --seed 20260905 --lengths 0=500,1=500,2=120,3=40,4=150 --whole-max 400 --tag main`

- Every one of the 92 shards of the two transitions (11 shards of the 10 -> 11 cells 11-17..11-23,
  81 shards of the 11 -> 12 cells 12-20..12-27) is touched.
- A shard with at most 400 parent records is replayed WHOLE (11-21, 11-22, 11-23, 12-25,
  12-26, 12-27: six shards, six complete cells).
- Every other shard gets ONE contiguous sub-range `[a, a+L)` of its parent range with `a` drawn
  uniformly by `random.Random(20260905)` in shard order (sorted by n, m, start), where L depends
  on the new-vertex degree d of the region the shard starts in: L = 120 for d = 2, 40 for d = 3,
  150 for d = 4, 500 for d <= 1. (L is a cost knob, nothing else: d = 3 costs ~14x a d = 2 parent.)
- Total: 7,503 parent records, 0.92% of the 815,215 in the two transitions; the parent records
  are the same frozen `_parents/<n>-<m>.g6` files udenum read (hashes in Section 5), at the
  same indices, with the same target (n, m), hence the same d per record and the same edge
  windows (the windows are encoded by which parent levels the `_parents` file concatenates,
  per `reach.json` and the aggregate manifests' `parent_sources`).

### 2.3 How udenum's output is restricted to a sub-range (the "restriction")

Shard outputs are kept (`shards/<n>-<m>/<shard_id>.g6`, hashed in the shard manifest; every
hash is re-checked by the driver). For a whole-shard replay the comparison is against the whole
file. For a sub-range `[a, b)` the driver recovers, for a udenum child G (canonical graph6), the
parent it was accepted from: delete the first minimum-degree vertex of G in canonical order (the
record is canonical, so that is the smallest-index minimum-degree vertex; any other choice nauty
could make is an automorphic image), canonicalise the remainder with enum2's own
`canonical_g6`, and look it up in the shard's parent records to get its index. udenum writes
children in parent order (single-threaded `process_parent` loop, `fputs` per accepted child),
so the children of parents `[a, b)` are one contiguous block of lines; the block start is found
by binary search on the recovered parent index, the block end by scanning forward, and then
EVERY line of the block is verified to recover to a parent index in `[a, b)` in non-decreasing
order, and the lines just outside the block to recover outside it. A child whose recovered
parent is not a record of the shard's range raises an error (none did). The restriction cannot
produce a false "identical": a udenum child of the range that the block missed would be
produced by enum2 and show up as `only enum2`; a udenum child wrongly included shows up as
`only udenum`.

The comparison per shard is then: parents replayed == b - a; the shard file's line count equals
the manifest's `children_written` and the file hash equals the manifest's; the block has no
duplicate line; the SET of block lines equals the SET of enum2's canonical children (enum2's
raw list is also counted: it exceeds the distinct count exactly when two Aut(H)-equivalent
neighbourhoods were both accepted, which the set removes, Section 1.3). "identical" means all
of these hold.
...
What it establishes. On a seeded sample touching every one of the 92 shards of the 10 -> 11 and
11 -> 12 transitions of the pruned (22,61) DAG (`enum/out-22/dag-22-61-f12`), an independently
written enumerator (Python + pynauty, different bad-set algorithm, no shared code beyond the
nauty library) reproduced udenum's output exactly: 7,503 parent records replayed (0.92% of the
815,215 parent records of the two transitions; 6,177 of them pass the minimum-degree guard,
0.97% of udenum's 636,488 processed parents), 57,355 canonical children (1.12% of the
5,128,203 children of the two transitions), zero children only on either side, in all 15 cells,
for every new-vertex degree d in {1, 2, 3, 4} on both 10- and 11-vertex hosts, including six
cells replayed whole (11-21, 11-22, 11-23, 12-25, 12-26, 12-27). The 4,326 duplicate
constructions enum2 made (Aut(H)-equivalent neighbourhoods) all collapsed onto children already
present, and udenum's blocks contained no duplicate, which is the empirical face of the
canonical-augmentation argument in Section 1.3. The comparison machinery was shown to go red on
a single flipped edge in one parent record and on a single forbidden graph removed from the list
(Section 4), so an agreement here is not an artefact of a comparison that cannot fail. The
semantics of the two enumerators were compared by reading (Section 1) and no difference was found
in the acceptance rule, the guard, or the definition of the bad family; the differences found
are in interface and algorithm only.

What it does not establish. It is a 0.92% sample of two transitions, not a replay of the DAG:
levels 13 and above were not touched, nor the seed levels (n <= 10, from geng), nor the reach
filter, the pruning, or any certificate. Thirteen of the 86 sub-ranges were vacuous (Section 3)
and 12-24's only range is among them, so the d = 4 path on 11-vertex hosts rests on 12-25, 12-26
and 12-27 alone (149 + 18 + 2 guard-passing parents). A bug in udenum confined to a region no
range hit (a particular parent shape, a shard beyond level 12, or a very rare bad-set pattern)
would not have been seen; what the sample does exclude is any systematic error in the
acceptance rule, canonical form, guard, or bad-family construction, since such an error would
have appeared in most of the 79 non-vacuous ranges. Both enumerators share nauty 2.8.8 for
canonical labelling and orbits, so an error inside nauty itself would be common to both and
invisible here. The measured rate of enum2 (Section 2.1) makes a larger fraction a matter of
budget, not design: the next step, if wanted, is `replay.py plan --seed 20260906 ...` restricted
to the 13 vacuous shards (an estimated 30 minutes on 4 cores) and then a whole-shard replay of
one dense d = 2 shard of 12-20 (about 4,000 core-seconds).
